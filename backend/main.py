...
def create_employee(employee: schemas.EmployeeCreate, db: Session = Depends(get_db)):
    db_employee = test.get_employee_by_email(db, email=employee.email)
    if db_employee:
        raise HTTPException(status_code=400, detail="Email already registered")
    return test.create_employee(db=db, employee=employee)

@app.get("/employees/", response_model=List[schemas.Employee])
def read_employees(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return test.get_employees(db, skip=skip, limit=limit)

@app.delete("/employees/{employee_id}", response_model=schemas.Employee)
def delete_employee(employee_id: int, db: Session = Depends(get_db)):
    db_employee = test.delete_employee(db, employee_id)
    if not db_employee:
...
def create_attendance(attendance: schemas.AttendanceCreate, db: Session = Depends(get_db)):
    return test.create_attendance(db=db, attendance=attendance)
